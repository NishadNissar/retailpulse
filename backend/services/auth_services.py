...
def register_step1(db: Session, data: Step1AccountCreate) -> Step1Response:
    ...
    1. Check email & phone uniqueness
    2. Hash password
    3. Save user with is_phone_verified=True  ← auto-verified for demo
    # 4. Generate OTP, save it, send via SMS  ← disabled for demo
    """
    ...
    # ── Create user ───────────────────────────────────────────────────────────
    # otp             = generate_otp()          # ← disabled for demo
    # otp_expiry_time = otp_expiry(minutes=10)  # ← disabled for demo
    ...
    new_user = User(
        ...
        phone             = data.phone,
        password_hash     = hash_password(data.password),
        # otp_code        = otp,               # ← disabled for demo
        # otp_expires_at  = otp_expiry_time,   # ← disabled for demo
    ...
    # send_otp_sms(new_user.phone, otp)  # ← disabled for demo
...
def register_step2(db: Session, data: Step2OTPVerify) -> Step2Response:
    """
    Auto-passes for demo — phone is already verified at step 1.
    # 1. Find user                             ← kept
    # 2. Check OTP matches and hasn't expired  ← disabled for demo
    # 3. Mark phone as verified, clear OTP     ← already done at step 1
    """
    user = _get_user_or_404(db, data.user_id)

    # ── Already verified (always true in demo) ────────────────────────────────
    if user.is_phone_verified:
        return Step2Response(
            message           = "Phone already verified. Continue to next step.",
            user_id           = user.id,
...
def resend_otp(db: Session, data: ResendOTPRequest) -> ResendOTPResponse:
    """Disabled for demo — phone is auto-verified at step 1."""
    user = _get_user_or_404(db, data.user_id)
    ...
    # otp                 = generate_otp()           # ← disabled for demo
    # user.otp_code       = otp                      # ← disabled for demo
    # user.otp_expires_at = otp_expiry(minutes=10)   # ← disabled for demo
    # db.commit()                                    # ← disabled for demo
    # send_otp_sms(user.phone, otp)                  # ← disabled for demo
